<?php
	...
	include_once '../includes/jqUploader/php/RecoveryClass.php';
	...
	$objetoDeArchivos->setTablaDeArchivos(array(
		...
		"campoDeImagenDePortada"=>"imagen_de_portada", // Este campo personalizado de la tabla de CH.
	...
	/* El siguiente método es opcional. Si en tu formulario no defines campos complementarios para los archivos subidos con el plugin, 
	no uses este método. */
	...
	/* El siguiente método es opcional. Si en tu página no hay campos asociados al plugin, no emplees este método. */
	...
	$fallo = $objetoDeArchivos->saveFiles('../../imagenes/trabajos'); // indicador de si ha habido fallo
	...
	foreach ($matrizDeImagenes as $clave=>$imagen) {
		...
		foreach ($matrizDeDatos as $dato) {
			if ($dato["nombre_de_dato"] == "portada") $matrizDeImagenes[$clave]["imagen_de_portada"] = $dato["valor_de_dato"];
	...
	foreach ($matrizDeImagenes as $imagen) {
		...
		/* Definimos el valor para imagen_de_portado en base al valor recibido o nsi no recibimos nada para que la variable no quede vacia */
		if ($imagen["imagen_de_portada"] == "P") {
			$portada = "P";
		} else {
			$portada = "N";
		}
		...
		$hacerConsulta->bindParam(":imagenDePortada", $portada);
		$hacerConsulta->execute(); // Se ejecuta la consulta.
		$hacerConsulta->closeCursor(); // Se libera el recurso.
