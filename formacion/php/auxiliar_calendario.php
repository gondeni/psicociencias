...
try {
  ...
  foreach ($sesiones as $keySesion => $sesion){
    ...
    $sesiones[$keySesion]['color_calendario'] = '';

    $consulta = "SELECT `idDeCurso`,`nombreDeCurso`,`color_calendario` FROM `cursos`";
    $hacerConsulta = $conexion->prepare($consulta);
    $hacerConsulta->execute();
    ...
    foreach($cursos as $curso){
      if($curso['idDeCurso'] == $sesiones[$keySesion]['id_de_curso']){
        ...
        $sesiones[$keySesion]['color_calendario'] = $curso['color_calendario'];
      }
    }
    ...
    $e['color'] = $sesiones[$keySesion]['color_calendario'];
    $e['allDay'] = true;
